...
def count_raised_fingers(hand_landmarks):
    ...
    # For the flipped image, thumb is considered raised if the tip (landmark[4]) is above knuckle (landmark[3])
    if hand_landmarks.landmark[4].y < hand_landmarks.landmark[3].y:
        fingers_raised += 1

    # Check for other fingers based on landmarks for each finger's tip and knuckle
    for finger_tip, finger_knuckle in zip([8, 12, 16, 20], [7, 11, 15, 19]):
        if hand_landmarks.landmark[finger_tip].y < hand_landmarks.landmark[finger_knuckle].y:
            fingers_raised += 1
